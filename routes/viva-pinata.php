...
$titre = getCategoryByID($connexion, 2);
$limit = ($base == $script ? 6 : null);
?>
<div class="container px-8 <?= ($base != $script ? "mb-12" : "") ?>">
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-12">
        <?php foreach (
            array_slice(shuffle_return(getAllProductsByCategorie($connexion, 2)), 0, $limit) as $produit
        ) : ?>
                    ...
                    <div class="flex flex-col h-full">
                        ...
                        <div class="flex flex-col p-6 gap-3 h-full">
                            ...
                            <div class="text-center relative w-full cursor-pointer overflow-hidden">
                                ...
                                <p class="absolute top-0 left-0 w-full text-lg font-semibold transition-opacity ease-in-out duration-300 opacity-0 group-hover:opacity-100 text-white bg-black rounded-xl">Adoptez <?= $produit["genre_produit"] ?> !</p>
                            </div>
                        </div>
                    </div>
                </a>
